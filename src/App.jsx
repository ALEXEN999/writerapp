      ...
      <div className={`absolute inset-0 flex items-center justify-center ${!char.imageUrl ? getBgColor(char.name || "") : 'bg-stone-100'}`}>
         {char.imageUrl ? (
            ...
            <span className="font-serif text-6xl md:text-8xl font-bold text-stone-400/50 select-none">
                {char.name ? String(char.name).substring(0, 1).toUpperCase() : "?"}
            </span>
         )}
      </div>
                            ...
                            {editingItem ? (
                                <div className="flex flex-wrap gap-2">
                                    {(data.species || []).map(sp => {
                                        ...
                                        const name = typeof sp === 'string' ? sp : (sp?.name || "Desconocido");
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {item.residentSpecies && item.residentSpecies.length > 0 ? item.residentSpecies.map((sp, i) => (
                                        <span key={i} className="px-3 py-1 bg-stone-900 text-amber-50 text-xs font-bold">
                                            {/* PROTECCIÓN CONTRA OBJETOS */}
                                            {typeof sp === 'string' ? sp : "Error: Objeto"}
                                        </span>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="text-[10px] text-stone-500 uppercase tracking-wider">{char.race}</div>
                                        </div>
