...
const EntityEditor = ({ listName, item, data, updateItem, deleteItem, setExpandedItemId, editingItem, setEditingItem }) => {
    ...
    const safeItem = typeof item === 'string' ? { id: item, name: item, residentSpecies: [] } : item;
                        ...
                        <div className="bg-white p-6 border border-stone-300 shadow-sm">
                            ...
                            {editingItem ? (
                                <div className="flex flex-wrap gap-2">
                                    {(data.species || []).map(sp => {
                                        // SAFE GUARD: Handle legacy strings or objects
                                        if (!sp) return null;
                                        const name = typeof sp === 'string' ? sp : String(sp.name || "Desconocido");
                                        const isSelected = (safeItem.residentSpecies || []).includes(name);
                                        return (
                                            <button key={sp.id || name} onClick={() => toggleSpeciesForWorld(name)} className={`px-3 py-1 text-xs font-bold border transition-colors ${isSelected ? 'bg-stone-800 text-white border-stone-800' : 'bg-white text-stone-500 border-stone-200'}`}>
                                                {name}
                                            </button>
                                        )
                                    })}
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {(safeItem.residentSpecies && safeItem.residentSpecies.length > 0) ? safeItem.residentSpecies.map((sp, i) => (
                                        <span key={i} className="px-3 py-1 bg-stone-900 text-amber-50 text-xs font-bold">
                                            {/* SAFE GUARD */}
                                            {typeof sp === 'string' ? sp : String(sp?.name || "Error")}
                                        </span>
                                    )) : <p className="text-xs text-stone-400 italic">Ninguna.</p>}
                                </div>
                            )}
                        </div>
                        <div className="bg-white p-6 border border-stone-300 shadow-sm">
                    ...
                    {isSpecies && (
                        <>
                           ...
                           <div className="bg-white p-6 border border-stone-300 shadow-sm">
                                ...
                                {editingItem ? (
                                    <select className="w-full p-3 bg-stone-50 border border-stone-300 text-sm font-serif focus:border-amber-500 outline-none" value={safeItem.worldOrigin || ""} onChange={(e) => updateItem(listName, safeItem.id, 'worldOrigin', e.target.value)}>
                                        <option value="">Selecciona un Mundo...</option>
                                        {(data.worlds || []).map(w => {
                                            if(!w) return null;
                                            const wName = typeof w === 'string' ? w : String(w.name || "Desconocido");
                                            return <option key={w.id || wName} value={wName}>{wName}</option>
                                        })}
                                    </select>
                                ) : (<p className="font-serif text-stone-700 text-sm">{safeItem.worldOrigin || "Desconocido"}</p>)}
                             </div>
                        </>
                    )}
