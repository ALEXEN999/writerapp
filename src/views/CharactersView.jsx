...
import { compressImage, getImageSizeKB } from "../utils/image";
...
const CharactersView = ({ data, updateData }) => {
  ...
  const dynamicRaces =
    getNames(data.species).length > 0 ? getNames(data.species) : DEFAULT_RACES;
  const dynamicWorlds =
    getNames(data.worlds).length > 0 ? getNames(data.worlds) : DEFAULT_WORLDS;
  ...
  const updateCharacterPlot = (charId, plotId, text) => {
    const char = (data.characters || []).find((c) => c.id === charId);
    if (!char) return;
    const newPlots = (char.characterPlots || []).map((p) =>
      p.id === plotId ? { ...p, text } : p
    );
  ...
  const addRelationship = (charId) => {
    ...
    const existing = (char.relationships || []).find(
      (r) => r.targetId === newRelationId
    );
    if (existing)
      return alert("Ya existe una relación con este personaje.");

    const newRel = { targetId: newRelationId, type: newRelationType };
    updateChar(charId, "relationships", [
      ...(char.relationships || []),
      newRel,
    ]);
    setNewRelationId("");
  ...
  const filteredChars = (data.characters || []).filter((char) => {
    if (filters.importance !== "Todas" && char.importance !== filters.importance)
      return false;
  ...
  const FilterSelect = ({ label, value, options, onChange }) => (
    <div className="flex flex-col w-full">
      <label className="text-[8px] font-bold text-stone-400 uppercase mb-1 ml-1 tracking-widest">
        {label}
      </label>
      <select
  ...
  const expandedChar = (data.characters || []).find(
    (c) => c.id === expandedCharId
  );
  ...
  if (expandedCharId && expandedChar) {
    ...
    return (
      ...
      >
        {/* CONTENIDO DEL EDITOR (sin flex-1 overflow-y-auto aquí, ya lo hace el overlay) */}
        <div className="flex flex-col items-center mb-8">
            ...
            {expandedChar.imageUrl && (
              <div className="absolute -bottom-6 left-0 right-0 text-center">
                <span className="text-[8px] text-stone-400 flex items-center justify-center gap-1">
                  <HardDrive size={8} />{" "}
                  {getImageSizeKB(expandedChar.imageUrl)} KB
                </span>
              </div>
            )}
            <input
          ...
          {isEditing ? (
            <input
              ...
              onChange={(e) =>
                updateChar(expandedChar.id, "name", e.target.value)
              }
              placeholder="Nombre del Personaje"
            />
          ) : (
            <h1 className="font-serif font-bold text-4xl text-center text-stone-900 tracking-tight">
              {expandedChar.name || "Sin Nombre"}
            </h1>
          )}
        </div>
            ...
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                <div className="flex flex-col">
                  <label className="text-[9px] font-bold text-stone-400 uppercase mb-1">
                    Importancia
                  </label>
                  <select
                    className="p-2 bg-stone-50 border border-stone-300 focus:border-amber-500 outline-none rounded-none"
                    value={expandedChar.importance}
                    onChange={(e) =>
                      updateChar(expandedChar.id, "importance", e.target.value)
                    }
                  >
                    {CHAR_IMPORTANCE.map((o) => (
                ...
                <div className="flex flex-col">
                  ...
                  <select
                    className="p-2 bg-stone-50 border border-stone-300 focus:border-amber-500 outline-none rounded-none"
                    value={expandedChar.race}
                    onChange={(e) =>
                      updateChar(expandedChar.id, "race", e.target.value)
                    }
                  >
                    {dynamicRaces.map((o) => (
                      <option key={o} value={o}>
                ...
                <div className="flex flex-col">
                  ...
                  <select
                    className="p-2 bg-stone-50 border border-stone-300 focus:border-amber-500 outline-none rounded-none"
                    value={expandedChar.world}
                    onChange={(e) =>
                      updateChar(expandedChar.id, "world", e.target.value)
                    }
                  >
                    {dynamicWorlds.map((o) => (
                      <option key={o} value={o}>
                ...
                <div className="flex flex-col">
                  ...
                  <select
                    className="p-2 bg-stone-50 border border-stone-300 focus:border-amber-500 outline-none rounded-none"
                    value={expandedChar.csm}
                    onChange={(e) =>
                      updateChar(expandedChar.id, "csm", e.target.value)
                    }
                  >
                    {CHAR_CSM.map((o) => (
                      <option key={o} value={o}>
                ...
                <div className="flex flex-col">
                  ...
                  <select
                    className="p-2 bg-stone-50 border border-stone-300 focus:border-amber-500 outline-none rounded-none"
                    value={expandedChar.noble}
                    onChange={(e) =>
                      updateChar(expandedChar.id, "noble", e.target.value)
                    }
                  >
                    {CHAR_NOBLE.map((o) => (
                      <option key={o} value={o}>
                    ...
                    <button
                      ...
                      onClick={() =>
                        updateChar(
                          expandedChar.id,
                          `show${sec.key}`,
                          !expandedChar[`show${sec.key}`]
                        )
                      }
                      className={`px-4 py-2 text-[10px] font-bold uppercase border transition-all flex items-center gap-2 ${
                        expandedChar[`show${sec.key}`]
                          ? "bg-stone-900 text-white border-stone-900"
                          : "bg-white text-stone-400 border-stone-200 hover:border-amber-500"
                      }`}
                    >
              ...
              <div className="pt-8 border-t border-stone-200">
                ...
                <div className="bg-stone-50 p-4 border border-stone-200 mb-4">
                  <div className="flex gap-2 mb-2">
                    <select
                      ...
                      value={newRelationId}
                      onChange={(e) =>
                        setNewRelationId(parseInt(e.target.value))
                      }
                    >
                      <option value="">Seleccionar Personaje...</option>
                      {(data.characters || [])
                        .filter((c) => c.id !== expandedChar.id)
                        .map((c) => (
                    ...
                    <div className="flex border border-stone-300">
                      <button
                        onClick={() => setNewRelationType("ally")}
                        className={`px-3 py-1 text-xs font-bold uppercase ${
                          newRelationType === "ally"
                      ...
                      <button
                        onClick={() => setNewRelationType("enemy")}
                        className={`px-3 py-1 text-xs font-bold uppercase ${
                          newRelationType === "enemy"
                    ...
                    <button
                      onClick={() => addRelationship(expandedChar.id)}
                      disabled={!newRelationId}
                      className="bg-stone-800 text-white px-3 py-1 uppercase text-xs font-bold"
                    >
                      Añadir
                    </button>
                  </div>
                ...
                <div className="space-y-2">
                  {(expandedChar.relationships || []).map((rel, idx) => {
                    const target = (data.characters || []).find(
                      (c) => c.id === rel.targetId
                    );
                    ...
                    return (
                      <div
                        key={idx}
                        className={`flex justify-between items-center p-3 border-l-4 shadow-sm bg-white ${
                          isAlly
                            ? "border-l-blue-500"
                            : "border-l-red-500"
                        }`}
                      >
                        ...
                        <button
                          onClick={() =>
                            removeRelationship(expandedChar.id, rel.targetId)
                          }
                ...
                <div className="space-y-4">
                  {(expandedChar.characterPlots || []).map((plot) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() =>
                          deleteCharacterPlot(expandedChar.id, plot.id)
                        }
              ...
              <div className="pt-10">
                <button
                  onClick={() => deleteChar(expandedChar.id)}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {(expandedChar.relationships || []).map((rel, idx) => {
                        ...
                        return (
                          <div
                            key={idx}
                            className={`flex items-center gap-3 p-3 border-l-4 shadow-sm bg-white ${
                              isAlly
                                ? "border-l-blue-500"
                                : "border-l-red-500"
                            }`}
                          >
                            ...
                            <div>
                              <div className="font-serif font-bold text-stone-800 leading-tight">
                                {target.name}
                              </div>
                              <div
                  ...
                  <div className="mt-8 pt-8 border-t border-stone-100">
                    ...
                    <ul className="space-y-3">
                      {(expandedChar.characterPlots || []).map((p) => (
                        <li
                          key={p.id}
                          className="p-4 bg-stone-50 border-l-4 border-amber-500 text-base font-serif text-stone-800 shadow-sm"
                        >
                          {p.text}
                        </li>
                      ))}
  ...
  // Vista normal (lista de personajes)
  return (
    <div className="p-4 md:p-8 pb-40 animate-in fade-in">
      <div className="mb-8 bg-stone-50 border border-stone-200 p-4 shadow-inner">
        ...
        <div className="grid grid-cols-3 gap-3 pb-2">
          <FilterSelect
            ...
            options={CHAR_IMPORTANCE}
            onChange={(v) => setFilters({ ...filters, importance: v })}
          />
          <FilterSelect
            label="Raza"
            value={filters.race}
            options={dynamicRaces}
            onChange={(v) => setFilters({ ...filters, race: v })}
          />
          <FilterSelect
            label="Mundo"
            value={filters.world}
            options={dynamicWorlds}
            onChange={(v) => setFilters({ ...filters, world: v })}
          />
          <FilterSelect
            label="CSM"
            value={filters.csm}
            options={CHAR_CSM}
            onChange={(v) => setFilters({ ...filters, csm: v })}
          />
          <FilterSelect
            label="Noble"
            value={filters.noble}
            options={CHAR_NOBLE}
            onChange={(v) => setFilters({ ...filters, noble: v })}
          />
        </div>
      </div>
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <PillarButton onClick={addChar} variant="gold" icon={Plus}>
          Añadir Personaje
        </PillarButton>
      </div>
