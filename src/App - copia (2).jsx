...
const EntityEditor = ({ listName, item, data, updateItem, deleteItem, setExpandedItemId, editingItem, setEditingItem }) => {
    ...
    const autoResidentSpecies = isWorld ? (data.species || []).filter(sp => {
        if(!sp) return false;
        const origins = sp.originWorlds || (sp.worldOrigin ? [sp.worldOrigin] : []);
    ...
    // Toggle para Especies: FIXED (Una sola actualización)
    const toggleWorldOrigin = (worldName) => {
        const currentOrigins = safeItem.originWorlds || (safeItem.worldOrigin ? [safeItem.worldOrigin] : []);
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-8 max-w-3xl mx-auto w-full">
                <div className="flex flex-col items-center mb-8">
                    ...
                    {editingItem ? (
                        ...
                        <h1 className="font-serif font-bold text-4xl text-stone-900 text-center tracking-tight">{safeName}</h1>
                    )}
                </div>
                ...
                <div className="bg-white border border-stone-200 p-8 shadow-sm space-y-10">
                    {isWorld && (
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {autoResidentSpecies.length > 0 ? autoResidentSpecies.map((sp, i) => (
                        ...
                        <div>
                             ...
                             {editingItem ? (
                                 <textarea 
                                    ...
                                    onChange={(e)=>updateItem(listName,safeItem.id,'diegesis',e.target.value)}
                                    placeholder="Reglas específicas de este mundo..."
                                 />
                             ) : (
                                 <p className="font-serif text-base text-stone-800 leading-relaxed whitespace-pre-wrap pl-1 border-l-2 border-amber-200">{safeItem.diegesis||"Sin datos."}</p>
                             )}
                        </div>
                        </>
                    )}
                    ...
                    <div>
                        ...
                        {editingItem ? (
                            <textarea 
                                ...
                                onChange={(e)=>updateItem(listName,safeItem.id,'description',e.target.value)} 
                                placeholder="Descripción general..." 
                            />
                        ) : (
                            <p className="font-serif text-base text-stone-800 leading-relaxed whitespace-pre-wrap pl-1 border-l-2 border-amber-200">{safeItem.description || "Sin descripción."}</p>
                        )}
                    </div>
                    ...
                    {isSpecies && (
                        <>
                           <div>
                                ...
                                {editingItem ? <textarea className="w-full p-4 bg-stone-50 border border-stone-300 focus:border-amber-500 focus:outline-none min-h-[100px] font-serif text-stone-700 text-sm rounded-none" value={safeItem.appearance||""} onChange={(e)=>updateItem(listName,safeItem.id,'appearance',e.target.value)} placeholder="Características físicas..." /> : <p className="font-serif text-base text-stone-800 leading-relaxed whitespace-pre-wrap pl-1 border-l-2 border-amber-200">{safeItem.appearance||"Sin definir."}</p>}
                           </div>
                           <div>
                                ...
                                {editingItem ? <textarea className="w-full p-4 bg-stone-50 border border-stone-300 focus:border-amber-500 focus:outline-none min-h-[100px] font-serif text-stone-700 text-sm rounded-none" value={safeItem.powers||""} onChange={(e)=>updateItem(listName,safeItem.id,'powers',e.target.value)} placeholder="Habilidades mágicas..." /> : <p className="font-serif text-base text-stone-800 leading-relaxed whitespace-pre-wrap pl-1 border-l-2 border-amber-200">{safeItem.powers || "Sin definir."}</p>}
                           </div>
                           
                           {/* MULTIPLES MUNDOS DE ORIGEN */}
                           <div>
                                <h4 className="text-[10px] font-bold text-stone-400 uppercase mb-4 flex items-center gap-2 border-b border-stone-100 pb-2"><Globe size={12}/> Mundos de Origen</h4>
                                {editingItem ? (
                                    <div className="flex flex-wrap gap-2">
                                        {(data.worlds || []).map(w => {
                                            if(!w) return null;
                                            ...
                                            return (
                                                <button 
                                                    ...
                                                    className={`px-3 py-1 text-xs font-bold border transition-colors flex items-center gap-2 ${isSelected ? 'bg-stone-800 text-white border-stone-800' : 'bg-white text-stone-500 border-stone-200 hover:border-amber-400'}`}
                                                >
                                        ...
                                        {(() => {
                                            ...
                                            if(currentOrigins.length > 0) {
                                                return currentOrigins.map(origin => (
                                                    <span key={origin} className="px-3 py-1 bg-stone-100 text-stone-600 text-xs font-bold border border-stone-200">{origin}</span>
                                                ));
                                            }
                                            return <p className="font-serif text-stone-400 italic text-sm">Desconocido</p>;
                                        })()}
                                    </div>
                                )}
                             </div>
                    ...
                    {editingItem && (
                         <div className="pt-8">
                            <button onClick={() => deleteItem(listName, safeItem.id)} className="w-full py-4 text-red-600 border border-red-200 hover:bg-red-50 uppercase text-xs font-bold flex items-center justify-center gap-2 transition-colors rounded-none">
...
const WorldView = ({ data, updateData }) => {
  ...
  const addItem = (listName) => {
    const newItem = { id: Date.now(), name: `Nuevo ${listName === 'species' ? 'Especie' : 'Mundo'}`, imageUrl: "", originWorlds: [] };
        ...
        <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                ...
                {worldsList.map(item => {
                    ...
                    const safeItem = typeof item === 'string' ? { id: item, name: item, imageUrl: null } : item;
                    return (
                        <div key={safeItem.id || Math.random()} onClick={() => { setExpandedItemId(safeItem.id); setActiveListType('worlds'); setEditingItem(false); }} className="aspect-square bg-white border border-stone-300 shadow-sm hover:shadow-md flex flex-col items-center justify-center p-4 text-center cursor-pointer relative group transition-all overflow-hidden hover:border-amber-400">
                            {safeItem.imageUrl ? (
                                <img src={safeItem.imageUrl} className="absolute inset-0 w-full h-full object-cover opacity-30 group-hover:opacity-50 transition-opacity" />
                            ) : (
                                <Globe size={24} className="text-stone-300 mb-3 group-hover:text-amber-400 transition-colors relative z-10"/>
                            )}
                            <h4 className="font-serif font-bold text-lg text-stone-800 leading-tight relative z-10">{String(safeItem.name || "Sin Nombre")}</h4>
        ...
        <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                ...
                {speciesList.map(item => {
                    ...
                    const safeItem = typeof item === 'string' ? { id: item, name: item, imageUrl: null } : item;
                    return (
                        <div key={safeItem.id || Math.random()} onClick={() => { setExpandedItemId(safeItem.id); setActiveListType('species'); setEditingItem(false); }} className="aspect-square bg-white border border-stone-300 shadow-sm hover:shadow-md flex flex-col items-center justify-center p-4 text-center cursor-pointer relative group transition-all overflow-hidden hover:border-amber-400">
                             {safeItem.imageUrl ? (
                                <img src={safeItem.imageUrl} className="absolute inset-0 w-full h-full object-cover opacity-30 group-hover:opacity-50 transition-opacity" />
                            ) : (
